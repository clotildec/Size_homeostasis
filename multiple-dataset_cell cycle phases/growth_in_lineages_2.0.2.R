...
data <- read.csv("I:/00_GROUP EXPERIMENTS/151125_allHela_chabbels_chambers/151126_only_4good_exp_HeLa_chambers/151126_group_150514_151116_151028_hela_fucci.csv",sep=",")

### prepare a clean dataframe ####
## parameters to enter ####
dir <- "I:/00_GROUP EXPERIMENTS/151127_group_hela_rsco_151016_151028/final-analysis-by-condition-rosco/"
title <- "group_HeLa_Fucci_rosco_151016_151028_analyzed_151127_ds2"
time_resolution <- 10 
data$cond_cell_type <- "HeLa_Fucci"
data$cond_date <- 150514
data$cond_vol <- NA
data$vol <- data$Vol
...
df<- data
df$sort_cond=df$cond ## chose the condition 
...
names=c(names1,names11,names2,names3,names4)
rm(names1,names11,names2,names3,names4)
df$sort_cond=df$cond_date ## chose the condition 
cond=unique(df$sort_cond)
final=1.6
initial=240
...
## save dataframe df1 !!carefull with info to enter here ####
df1$dvdt<-df1$dv_tot/df1$dt_tot
df1$cond_cell_type <- as.character(unique(data$cond_cell_type))
df1$cond_date <- unique(data$cond_date)
df1$cond_vol <- as.character(unique(data$cond_vol))
...
df<-data
df$sort_cond <-df$cond ## put in that column the sorting condition
cond=unique(df$sort_cond)
...
## or make dataframe directly with df1 from above:
# ccl_complete=df$dt_tot
# cell_number_complete=df$number_in_lineage
# cond_complete=df$sort_cond
# lineage_complete=df$lineage
# generation_complete=df$generation
# ti_complete=df$t_240
# tf_complete=df$t_1.6
...
grid.arrange(pa,hist1,pb,hist2,ncol=2)
dev.off()
...
for (i in cond){
  print(i)
  x=df$ti[which(df$sort_cond==i & is.na(df$ti)==FALSE & is.na(df$vi)==FALSE)]
  y=df$vi[which(df$sort_cond==i & is.na(df$vi)==FALSE & is.na(df$ti)==FALSE)]
  test=lm(y~x)
  xlab=paste(xlab,"\n",i,", lm: R2=",signif(summary(test)$r.squared,a),", p=",signif(anova(test)$'Pr(>F)',a), ", n= ",length(x),sep="")
  rm(x,y,test)
}
test=lm(df$vi~df$ti)
p4 <- ggplot(df,aes(x=ti,y=vi, color=factor(sort_cond))) +
  geom_point() +
  labs(title= title, y="vi [µm3]", x=paste("Time_i [hours]","\n lm: ","R2=",signif(summary(test)$r.squared,2),
                                           ", p=",signif(anova(test)$'Pr(>F)',2),
                                           ", n=",length(df$vi[which(is.na(df$vi)==F)]),xlab)) +
  scale_colour_discrete (name="date") +
  #stat_smooth(method="lm")+
  theme_bw()
sink(file=paste(dir,title,"ctrl_lm_vi_ti.txt",sep=""))
summary(test)
sink() 
rm(test)

xlab=" "
mat=matrix(NA,nrow=length(cond),ncol=3)
j=0
for (i in cond){
  j=j+1
  y=df$vi[which(df$sort_cond==i & is.na(df$vi)==FALSE)]
  xlab=paste(xlab,"\n",i,": mean=",signif(mean(y),a),', sd=',signif(sd(y),a), ", n=",length(y),sep="")
  mat[j,1]=i
  mat[j,2]=mean(y)
  mat[j,3]=sd(y)
}
colnames(mat)=c("cond","mean","sd")
table=tableGrob(mat,equal.width = F)
grid.newpage()
h <- grobHeight(table)
w <- grobWidth(table)
title_temp <- textGrob(paste(title,"\nsvi sorting by ",name_tri), y=unit(0.5,"npc") + 0.6*h, 
                       vjust=0, gp=gpar(fontsize=15))
gt <- gTree(children=gList(table, title_temp))
tiff(paste(dir,title,"_vi_table_sorting_",name_tri,".tiff",sep=""), width=500,height=200)
grid.draw(gt)
dev.off()

hist4 <- ggplot(df,aes(x=vi,fill=as.factor(cond)))+
  geom_histogram(position="dodge")+
  theme_bw()+
  labs(title=title,
       fill=title_cond,
       x= paste('vi [µm3]','\n all conditions: ', 'mean=',signif(mean(df$vi[which(is.na(df$vi)==F)]),a*2)," µm3",
...
for (i in cond){
  j=j+1
  y=df$vf[which(df$sort_cond==i & is.na(df$vf)==FALSE)]
  xlab=paste(xlab,"\n",i,": mean=",signif(mean(y),a),', sd=',signif(sd(y),a), ", n=",length(y),sep="")
  mat[j,1]=i
  mat[j,2]=mean(y)
  mat[j,3]=sd(y)
...
h <- grobHeight(table)
w <- grobWidth(table)
title_temp <- textGrob(paste(title,"\nsvf sorting by ",name_tri), y=unit(0.5,"npc") + 0.6*h, 
                       vjust=0, gp=gpar(fontsize=15))
gt <- gTree(children=gList(table, title_temp))
tiff(paste(dir,title,"_vf_table_sorting_",name_tri,".tiff",sep=""), width=500,height=200)
grid.draw(gt)
dev.off()

hist1 <- ggplot(df,aes(x=vf,fill=as.factor(cond)))+
  geom_histogram(position="dodge")+
  theme_bw()+
  labs(title=title,
       fill=title_cond,
       x= paste('Vf [µm3]','\n all conditions: ', 'mean=',signif(mean(df$vf[which(is.na(df$vf)==F)]),a*2)," µm3",
...
for (i in cond){
  j=j+1
  y=df$dv[which(df$sort_cond==i & is.na(df$dv)==FALSE)]
  xlab=paste(xlab,"\n",i,": mean=",signif(mean(y),a),' sd=',signif(sd(y),a), ", n=",length(y),sep="")
  mat[j,1]=i
  mat[j,2]=mean(y)
  mat[j,3]=sd(y)
...
h <- grobHeight(table)
w <- grobWidth(table)
title_temp <- textGrob(paste(title,"\nsdv sorting by ",name_tri), y=unit(0.5,"npc") + 0.6*h, 
                       vjust=0, gp=gpar(fontsize=15))
gt <- gTree(children=gList(table, title_temp))
tiff(paste(dir,title,"_dv_table_sorting_",name_tri,".tiff",sep=""), width=500,height=200)
grid.draw(gt)
dev.off()

hist2 <- ggplot(df,aes(x=dv,fill=as.factor(cond)))+
  geom_histogram(position="dodge")+
  theme_bw()+
  labs(title=title,
       fill=title_cond,
       x= paste('DV [µm3]','\n all conditions: ', 'mean=',signif(mean(df$dv[which(is.na(df$dv)==F)]),a*2)," µm3",
...
for (i in cond){
  j=j+1
  y=df$dvdt[which(df$sort_cond==i & is.na(df$dvdt)==FALSE)]
  xlab=paste(xlab,"\n",i,": mean=",signif(mean(y),a),' sd=',signif(sd(y),a), ", n=",length(y),sep="")
  mat[j,1]=i
  mat[j,2]=mean(y)
  mat[j,3]=sd(y)
...
h <- grobHeight(table)
w <- grobWidth(table)
title_temp <- textGrob(paste(title,"\nsdvdt sorting by ",name_tri), y=unit(0.5,"npc") + 0.6*h, 
                       vjust=0, gp=gpar(fontsize=15))
gt <- gTree(children=gList(table, title_temp))
tiff(paste(dir,title,"_dvdt_table_sorting_",name_tri,".tiff",sep=""), width=500,height=200)
grid.draw(gt)
dev.off()

hist3 <- ggplot(df,aes(x=dvdt,fill=as.factor(cond)))+
  geom_histogram(position="dodge")+
  theme_bw()+
  labs(title=title,
       fill=title_cond,
       x= paste('DV/DT [µm3/hrs]','\n all conditions: ', 'mean=',signif(mean(df$dvdt[which(is.na(df$dv)==F)]),a*2)," µm3",
                ", n=",length(df$dvdt[which(is.na(df$dv)==F)]),xlab,sep=""))

# save in figure ####
tiff(paste(dir,title,"_plot_vol_ctrl.tiff",sep=""), width=1000,height=1000)
grid.arrange(p4,hist4,p1,hist1,p3,hist3,p2,hist2,ncol=2)
dev.off()

######## histograms and plots of v, dt, dv for each phase of the cell cycle ####
...
hist_v<-function(l){
  ...
  t=list()
  for (i in cond){
    xlab=paste(xlab,"\n",i,": mean=",signif(mean(d$y[which(d$z==i)]),a),', sd=',signif(sd(d$y),a*2), ", n=",length(d$y[which(d$z==i)]),sep="")
  }
  if (length(cond)==2){
   t1=d$y[which(d$z==cond[1])]
   t2=d$y[which(d$z==cond[2])]
   xlab=paste(xlab,"\nt.Test,p=",signif(t.test(t1,t2)$p.value,a),sep="")
  }else{}
  ...
  hist<- ggplot(d,aes(x=y,fill=as.factor(z)))+
    ...
    labs(title=paste("volume_",names[l],sep=""),
         fill=title_cond,
         x= paste('Volume [µm3]','\n all conditions: ', 'mean=',signif(mean(d$y),a*2),'sd=',signif(sd(d$y),a*2)," µm3",
...
# tiff(paste(dir,title,title_cond,"_hist_vol.tiff",sep=""), width=500,height=500)
# grid.arrange(hist_v(1),hist_v(2),ncol=1, main=title)
# dev.off()
...
dev.off()
# tiff(paste(dir,title,title_cond,"_hist_dv.tiff",sep=""), width=500,height=500)
# grid.arrange(hist_dv(1),ncol=1, main=title)
# dev.off()
...
hist_dv<-function(l){
  xlab=" "
  K=which(colnames(df)==paste("dv_",names_phase[l],sep=""))
  L=which(colnames(df)==paste("v_",t_phase[l],sep=""))
  x=df[,L][which(is.na(df[,K])==F)]
  y=df[,K][which(is.na(df[,K])==F)]
  z = df$sort_cond[which(is.na(df[,K])==F)]
  d = data.frame(x,y,z)
  for (i in cond){
    ...
    xlab=paste(xlab,"\n",i,", lmrob: R2=",signif(summary(testb)$r.squared,a),", a=",signif(coef(testb)[2],a), ", b=", signif(coef(testb)[1],a),", lm: p=",signif(anova(test)$'Pr(>F)',a),", Pearson=",signif(cor(x1,y1),a), ", n= ",length(x1),sep="")
    print(i)
    sink(file=paste(dir,title,"_lm_lmrob_coeff-corr_dv_vi",title_cond,"_",names_cond[i],".txt",sep=""))
    summary(test)
    summary(testb)
    cor.test(x,y)
    sink()
  ...
  test = lm(y~x)
  testb = lmrob(y~x)
  plot <- ggplot(d,aes(x=x,y=y, color=factor(z))) +
    geom_point() +
    labs(title= title, y=paste("DV_",names_phase[l]," [µm3]",sep=""), 
         x=paste("Vol_",names_t_phase[l], "[µm3]","\nlm:","R2=",signif(summary(test)$r.squared,a),", p=",signif(anova(test)$'Pr(>F)',a),
                 ", n=",length(x), 
                 ", lmrob: a=",signif(coef(testb)[2],a), ", b=", signif(coef(testb)[1],a),
                 "\n Pearson: r=", signif(cor(x,y),a), ", p=",signif(cor.test(x,y)$p.value,a),sep=""))+
    scale_colour_discrete (name=name_tri) +
    geom_abline(intercept = coef(testb)[1], slope = coef(testb)[2])+
    theme_bw()
}
tiff(paste(dir,title,title_cond,"_plot_dvvi.tiff",sep=""), width=500,height=750)
grid.arrange(hist_dv(1),hist_dv(2),hist_dv(3),ncol=1, main=title)
dev.off()
...
grid.arrange(hist_dv(1),hist_dv(2),hist_dv(3),ncol=1, main=title)
dev.off()
# tiff(paste(dir,title,title_cond,"_hist_dt.tiff",sep=""), width=500,height=500)
# grid.arrange(hist_dv(1),ncol=1, main=title)
# dev.off()
...
hist_dv<-function(l){
  ...
  for (i in cond){
    ...
    xlab=paste(xlab,"\n",i,", lmrob: R2=",signif(summary(testb)$r.squared,a),", a=",signif(coef(testb)[2],a), ", b=", signif(coef(testb)[1],a),", lm: p=",signif(anova(test)$'Pr(>F)',a),", Pearson=",signif(cor(x1,y1),a), ", n= ",length(x1),sep="")
    print(i)
    sink(file=paste(dir,title,"_lm_lmrob_coeff-corr_dt_vi",title_cond,"_",names_cond[i],".txt",sep=""))
    summary(test)
    summary(testb)
    cor.test(x,y)
    sink()
  ...
  plot <- ggplot(d,aes(x=x,y=y, color=factor(z))) +
    geom_point() +
    labs(title= title, y=paste("DT_",names_phase[l]," [hrs]",sep=""), 
         x=paste("Vol_",names_t_phase[l], "[µm3]","\nlm:","R2=",signif(summary(test)$r.squared,a),", p=",signif(anova(test)$'Pr(>F)',a),
                 ", n=",length(x), 
                 ", lmrob: a=",signif(coef(testb)[2],a), ", b=", signif(coef(testb)[1],a),
...
# tiff(paste(dir,title,title_cond,"_plot_dtvi.tiff",sep=""), width=1000,height=1000)
# grid.arrange(hist_dv(1),hist_dv(2),hist_dv(3),hist_dv(4),hist_dv(5),hist_dv(6),ncol=2, main=title)
# dev.off()
tiff(paste(dir,title,title_cond,"_plot_dtvi.tiff",sep=""), width=500,height=750)
grid.arrange(hist_dv(1),hist_dv(2),hist_dv(3),ncol=1, main=title)
dev.off()
# tiff(paste(dir,title,title_cond,"_plot_dtvi.tiff",sep=""), width=500,height=500)
# grid.arrange(hist_dv(1),ncol=1, main=title)
# dev.off()
...
grid.arrange(hist_dv(1),hist_dv(2),hist_dv(3),ncol=1, main=title)
dev.off()
# tiff(paste(dir,title,title_cond,"_hist_dvdt.tiff",sep=""), width=500,height=500)
# grid.arrange(hist_dv(1),ncol=1, main=title)
# dev.off()
...
hist_dv<-function(l){
  xlab=" "
  K=which(colnames(df)==paste("v_",t_phase2[l],sep=""))
  L=which(colnames(df)==paste("v_",t_phase[l],sep=""))
  x=df[,L][which(is.na(df[,K])==F & is.na(df[,L])==F)]
  y=df[,K][which(is.na(df[,K])==F & is.na(df[,L])==F)]
  z = df$sort_cond[which(is.na(df[,K])==F & is.na(df[,L])==F)]
  d = data.frame(x,y,z)
  for (i in cond){
    x1=d$x[which(d$z==i)]
    y1=d$y[which(d$z==i)]
    ...
    testb=lmrob(y1~x1)
    #xlab=paste(xlab,"\n",i,", lm: R2=",signif(summary(test)$r.squared,a),", p=",signif(anova(test)$'Pr(>F)',a),",lmrob: a=",signif(coef(testb)[2],a), ", b=", signif(coef(testb)[1],a), ", n= ",length(x),sep="")
    xlab=paste(xlab,"\n",i,", lmrob: R2=",signif(summary(testb)$r.squared,a),", a=",signif(coef(testb)[2],a), ", b=", signif(coef(testb)[1],a),", lm: p=",signif(anova(test)$'Pr(>F)',a), ", n= ",length(x1),sep="")
    print(i)
    sink(file=paste(dir,title,"_lm_lmrob_coeff-corr_dt_vi",title_cond,"_",names_cond[i],".txt",sep=""))
    summary(test)
    summary(testb)
  ...
  plot <- ggplot(d,aes(x=x,y=y, color=factor(z))) +
    geom_point() +
    labs(title= title, y=paste("Vf_",names_phase[l]," [µm3]",sep=""), 
         x=paste("Vol_",names_t_phase[l], "[µm3]","\nlm:","R2=",signif(summary(test)$r.squared,a),", p=",signif(anova(test)$'Pr(>F)',a),
                 ", n=",length(x), 
                 ", lmrob: a=",signif(coef(testb)[2],a), ", b=", signif(coef(testb)[1],a),
                 "\n Pearson: r=", signif(cor(x,y),a), ", p=",signif(cor.test(x,y)$p.value,a),
...
# grid.arrange(hist_dv(1),hist_dv(2),hist_dv(3),hist_dv(4),hist_dv(5),hist_dv(6),ncol=2, main=title)
# dev.off()
tiff(paste(dir,title,title_cond,"_plot_vfvi.tiff",sep=""), width=500,height=750)
grid.arrange(hist_dv(1),hist_dv(2),hist_dv(3),ncol=1, main=title)
dev.off()
# tiff(paste(dir,title,title_cond,"_plot_dtvi.tiff",sep=""), width=500,height=500)
# grid.arrange(hist_dv(1),ncol=1, main=title)
...
grid.arrange(p2,p1,p3,p4,p5,p6,p7,p8,p9,ncol=2)
dev.off()
...
b = 240
df <- data
cond=unique(data$cond_date)
df$sort_cond=df$cond_date
name_sort_cond="cond_date"
sort_cond="sort_cond"
...
for (k in cond){
  for (i in unique(df$lineage[which(df$sort_cond==k)])){
    for (j in unique(df$number_in_lineage[which(df$lineage==i & df$number_in_lineage %%2==F & df$sort_cond==k)])){
      if (m %in% df$cycle_stage[which(df$lineage==i & df$number_in_lineage==j & df$sort_cond==k)]
          && m %in% df$cycle_stage[which(df$lineage==i & df$number_in_lineage==j+1 & df$sort_cond==k)]
          && b %in% df$cycle_stage[which(df$lineage==i & df$number_in_lineage==j & df$sort_cond==k)]
          && b %in% df$cycle_stage[which(df$lineage==i & df$number_in_lineage==j+1 & df$sort_cond==k)]
          && is.na(df$vol[which(df$lineage==i & df$number_in_lineage==j & df$cycle_stage==m & df$sort_cond==k)])==F
          && is.na(df$vol[which(df$lineage==i & df$number_in_lineage==j+1 & df$cycle_stage==m & df$sort_cond==k)])==F
          && is.na(df$vol[which(df$lineage==i & df$number_in_lineage==j & df$cycle_stage==b & df$sort_cond==k)])==F
          && is.na(df$vol[which(df$lineage==i & df$number_in_lineage==j+1 & df$cycle_stage==b & df$sort_cond==k)])==F){
        vf_1 = c(vf_1,df$vol[which(df$lineage==i & df$number_in_lineage==j & df$cycle_stage==m & df$sort_cond==k)])
        vf_2 = c(vf_2,df$vol[which(df$lineage==i & df$number_in_lineage==j+1 & df$cycle_stage==m & df$sort_cond==k)])
        vi_1 = c(vi_1,df$vol[which(df$lineage==i & df$number_in_lineage==j & df$cycle_stage==b & df$sort_cond==k)])
        vi_2 = c(vi_2,df$vol[which(df$lineage==i & df$number_in_lineage==j+1 & df$cycle_stage==b & df$sort_cond==k)])
        dt_1 = c(dt_1,df$Time[which(df$lineage==i & df$number_in_lineage==j & df$cycle_stage==m & df$sort_cond==k)]-
                   df$Time[which(df$lineage==i & df$number_in_lineage==j & df$cycle_stage==b & df$sort_cond==k)])
        dt_2 = c(dt_2,df$Time[which(df$lineage==i & df$number_in_lineage==j+1 & df$cycle_stage==m & df$sort_cond==k)]
                 - df$Time[which(df$lineage==i & df$number_in_lineage==j+1 & df$cycle_stage==b & df$sort_cond==k)])
        dvdt_1 = c(dvdt_1, ((df$vol[which(df$lineage==i & df$number_in_lineage==j & df$cycle_stage==m & df$sort_cond==k)]-
                               df$vol[which(df$lineage==i & df$number_in_lineage==j & df$cycle_stage==b & df$sort_cond==k)])/
                              ((df$Time[which(df$lineage==i & df$number_in_lineage==j & df$cycle_stage==m & df$sort_cond==k)]-
                                  df$Time[which(df$lineage==i & df$number_in_lineage==j & df$cycle_stage==b & df$sort_cond==k)])/60)))
        dvdt_2=c(dvdt_2, ((df$vol[which(df$lineage==i & df$number_in_lineage==j+1 & df$cycle_stage==m & df$sort_cond==k)]-
                             df$vol[which(df$lineage==i & df$number_in_lineage==j+1 & df$cycle_stage==b & df$sort_cond==k)])/
                            ((df$Time[which(df$lineage==i & df$number_in_lineage==j+1 & df$cycle_stage==m & df$sort_cond==k)]
                              - df$Time[which(df$lineage==i & df$number_in_lineage==j+1 & df$cycle_stage==b & df$sort_cond==k)])/60)))
        lineage = c(lineage,i)
        number_in_lineage = c(number_in_lineage,j)
        sort_cond=c(sort_cond,k)
      }
    }
  }
}
...
### plots ####
df <- data.frame(vf_1,vf_2,vi_1,vi_2,dt_1,dt_2, lineage, number_in_lineage,sort_cond)
...
### save figure ####
tiff(paste(dir,title,"_ratio_vol_sisters_i_f_boxplot.tiff",sep=""), width=300,height=300)
...
### make df and save and make boxplot ####
...
### save figure ####
tiff(paste(dir,title,"_ratio_normalized_vol_sisters_i_f_boxplot.tiff",sep=""), width=300,height=300)
